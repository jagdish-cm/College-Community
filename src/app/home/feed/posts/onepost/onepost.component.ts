...
export class OnepostComponent implements OnInit {
  ...
  setAuthForPost() {
    ...
    if (this.post.likedBy?.indexOf(this.curUser._id) !== -1) {
      this.userLiked = true;
      console.log('post liked ' + this.post.postContent);
    }
  }
  ...
  onApproveReject(status: number) {
    ...
    this.postService.approveOrRejectPost(data).subscribe((res) => {
      console.log(res);
      console.log(status);
      if (status == 1) {
        ...
        this.messageService.add({
          ...
          detail: 'Post will not be visible to other users!',
        });
        setTimeout(() => {
